...
Public Class Frm_Eval_segun_codigo
    ...
    'Private Property gstrFC_forestal As String
    ...
    Private Sub Frm_Eval_segun_codigo_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        lblSecPub.Enabled = False
        lblSecPub.Visible = False
        cboSecPub.Enabled = False
        cboSecPub.Visible = False
        ...
        If glo_int_Perfil <> 0 Then
            ...
            If m_form.DialogResult = Windows.Forms.DialogResult.OK Or gloint_Acceso = "1" Then
                ...
                'nuevo(OP - 30)
                Dim dRow6 As DataRow
                dRow6 = lodtbCboTipo.NewRow
                dRow6.Item("CODIGO") = "OP_30"
                dRow6.Item("DESCRIPCION") = "DM Division"
                lodtbCboTipo.Rows.Add(dRow6)

                'nuevo(OP - 31)
                Dim dRow7 As DataRow
                dRow7 = lodtbCboTipo.NewRow
                dRow7.Item("CODIGO") = "OP_31"
                dRow7.Item("DESCRIPCION") = "Mantenimiento DM_Acceditario"
                lodtbCboTipo.Rows.Add(dRow7)
            ...
            ''''''''''  COMBOBOX  ACCEDITARIOS

            cboSecPub.Visible = False
            cboSecPub.Enabled = False

            Dim dt_secpub As New DataTable  '
            Dim tipo As String = "2"
            dt_secpub = cls_Oracle.FT_DATA_ACCEDITARIO("", tipo)

            Dim lodtvsecpub As New DataView(dt_secpub)
            Me.cboSecPub.DataSource = lodtvsecpub
            Me.cboSecPub.DisplayMember = "PL_TIPPUB"
            Me.cboSecPub.ValueMember = "pl_codpub"
            'Me.cboFotomicrografia.ValueMember = "CODIGO"
    ...
    Private Sub Inicializa_Layer_DM(ByVal p_Tipo As String)
        Select Case p_Tipo
            Case "OP_7", "OP_10", "OP_8", "OP_13"
                ...
                '    Me.clbLayer.Items.Add("Zona Reservada")
                '    Me.clbLayer.Items.Add("Area Urbana")
                '    Me.clbLayer.Items.Add("Catastro Forestal")
            ...
            Case "OP_12", "OP_11"
                ...
                Me.clbLayer.SetItemCheckState(0, Windows.Forms.CheckState.Indeterminate)
                Me.clbLayer.SetItemCheckState(1, Windows.Forms.CheckState.Indeterminate)
                Me.clbLayer.SetItemCheckState(2, Windows.Forms.CheckState.Indeterminate)
            Case "OP_3", "OP_9"
                Me.clbLayer.Items.Add("Zona Reservada")
                Me.clbLayer.Items.Add("Area Urbana")
                Me.clbLayer.Items.Add("Limite Distrital")
    ...
    Private Sub btnBuscar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBuscar.Click
        ...
        If cbo_tipo.SelectedValue = "OP_29" Then
            ...
            'v_sistema = "WGS-84"
            'V_Sistemasele = "02"
            v_opcion_modulo = "OP_29"
            'v_equivalente = "02"

            Try
                Kill(glo_pathTMP & "\Acumulacion.shp")
                Kill(glo_pathTMP & "\Acumulacion.dbf")
                Kill(glo_pathTMP & "\Acumulacion.shx")
                Kill(glo_pathTMP & "\Acumulacion.prj")
                Kill(glo_pathTMP & "\Acumulacion.sbn")
                Kill(glo_pathTMP & "\Acumulacion.sbx")
            Catch ex As Exception

            End Try
        End If

        If cbo_tipo.SelectedValue = "OP_30" Then
            'v_sistema = "PSAD-56"
            'V_Sistemasele = "01"

            'v_sistema = "WGS-84"
            'V_Sistemasele = "02"
            v_opcion_modulo = "OP_30"
            'v_equivalente = "02"

            Try
                Kill(glo_pathTMP & "\Division.shp")
                Kill(glo_pathTMP & "\Division.dbf")
                Kill(glo_pathTMP & "\Division.shx")
                Kill(glo_pathTMP & "\Division.prj")
                Kill(glo_pathTMP & "\Division.sbn")
                Kill(glo_pathTMP & "\Division.sbx")
            Catch ex As Exception
        ...
        If v_sistema = "PSAD-56" Then
            ...
            gstrFC_forestal = "GPO_CFO_CATASTRO_FORESTAL_P"
            gstrFC_Frontera_WGS = "GPO_FRO_FRONTERA_PSAD_"
            gstrFC_Cuadriculas = "DATA_GIS.GPO_CUA_CUADRICULAS_"
        ...
        ElseIf v_sistema = "WGS-84" Then
            ...
            gstrFC_forestal = "GPO_CFO_CATASTRO_FORESTAL_W"
            gstrFC_Frontera_WGS = "GPO_FRO_FRONTERA_WGS_"
            gstrFC_Cuadriculas = "DATA_GIS.GPO_CUA_CUADRICULAS_WGS_"
        End If
        ...
        Select Case Me.cbo_tipo.SelectedValue 'tipo_seleccion
            ...
            Case "OP_11", "OP_2", "OP_5", "OP_6", "OP_16", "OP_18", "OP_19", "OP_20", "OP_29", "OP_30"
                ...
                If tipo_seleccion = "OP_11" Or tipo_seleccion = "OP_2" Or tipo_seleccion = "OP_18" Or tipo_seleccion = "OP_20" Or tipo_seleccion = "OP_29" Or tipo_seleccion = "OP_30" Then
                ...
                Dim lostrRegistro As String = "" : Dim lodbListaDM As New DataTable : Dim lodbListaValDM As New DataTable
                ...
                If CType(lostrRegistro, Integer) > 150 Then
                    MsgBox("Sea Ud. mas específico en la consulta, hay " & lostrRegistro & " Registro(s)", MsgBoxStyle.Information, "[SIGCATMIN]")
                    Exit Sub
                End If

                If cbo_tipo.SelectedValue = "OP_29" Then
                    Dim tipo_ac As String = "8"
                    lodbListaValDM = cls_Oracle.FT_DATA_ACUMULACION(txtConsulta.Text, tipo_ac)
                    Select Case lodbListaValDM.Rows.Count
                        Case 0
                            MsgBox("No Existe ningún Registo de Acumulación con esta consulta " & txtConsulta.Text, MsgBoxStyle.Information, "[SIGCATMIN]")
                            Exit Sub
                    End Select
                ElseIf cbo_tipo.SelectedValue = "OP_30" Then
                    Dim tipo_ac As String = "9"
                    lodbListaValDM = cls_Oracle.FT_DATA_ACUMULACION(txtConsulta.Text, tipo_ac)
                    Select Case lodbListaValDM.Rows.Count
                        Case 0
                            MsgBox("No Existe ningún Registo de División con esta consulta " & txtConsulta.Text, MsgBoxStyle.Information, "[SIGCATMIN]")
                            Exit Sub
                    End Select
                End If
                ...
                ElseIf cbo_tipo.SelectedValue = "OP_29" Then
                    Dim tipo_ac As String = "21"
                    lodbListaDM = cls_Oracle.FT_DATA_ACUMULACION(txtConsulta.Text, tipo_ac)
                ElseIf cbo_tipo.SelectedValue = "OP_30" Then
                    Dim tipo_ac As String = "12"
                    lodbListaDM = cls_Oracle.FT_DATA_ACUMULACION(txtConsulta.Text, tipo_ac)
                Else
                ...
                Select Case lodbListaDM.Rows.Count
                    Case 0
                        MsgBox("No Existe ningún Registo con esta consulta " & txtConsulta.Text, MsgBoxStyle.Information, "[SIGCATMIN]")
                        DialogResult = Windows.Forms.DialogResult.Cancel
                    Case Else
    ...
    Private Sub btnGraficar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGraficar.Click
        ...
        Select Case cbo_tipo.SelectedValue
            Case "OP_11" 'Evaluación
                ...
                GloInt_Opcion = 1
            Case "OP_12", "OP_29", "OP_30" 'Simulacion / Acumulación / División
                conta_botones_evaluacion = conta_botones_evaluacion + 1
                GloInt_Opcion = 1
            Case "OP_5" 'carta ign
                conta_botones_evaluacion = conta_botones_evaluacion + 1
        ...
        Select Case Me.cbo_tipo.SelectedValue
            ...
            Case "OP_7", "OP_10", "OP_8"
                ...
                Catch ex As Exception
                End Try
            Case "OP_12", "OP_29", "OP_30"  'simulacion
                ...
                If cbo_tipo.SelectedValue = "OP_12" Then
                ...
                If v_opcion_modulo = "OP_29" Then
                    cls_catastro.Quitar_Layer("Acumulacion", m_Application)
                ElseIf v_opcion_modulo = "OP_30" Then
                    cls_catastro.Quitar_Layer("Division", m_Application)
                End If
            ...
            Case "OP_29", "OP_30"   ' ACUMULACION, DIVISION
                ...
                pMxDoc = m_Application.Document
                pMap = pMxDoc.FocusMap
                glo_xMin = gloEsteMin - 5000
                glo_xMax = gloEsteMax + 5000
                glo_yMin = gloNorteMin - 5000
                glo_yMax = gloNorteMax + 5000
                caso_consulta = "CATASTRO MINERO"
                If pMap.Name <> "CATASTRO MINERO" Then
                    cls_planos.buscaadataframe(caso_consulta, False)
                    If valida = False Then
                ...
                cls_catastro.Consulta_XY_AC(m_Application, clbLayer, txtExiste)
    ...
    Private Sub visualizar_datos_dm(ByVal p_consulta As String, ByVal p_App As IApplication)
        ...
        Select Case Me.cbo_tipo.SelectedValue
            ...
            Case Else
                If v_sistema = "PSAD-56" Then  'PROCEDIMIENTO PARA OBTENER DM EN DATUN 56
                    ...
                    ElseIf cbo_tipo.SelectedValue = "OP_29" Then
                        Dim tipo_ac As String = "51"
                        'lodtbDatos = cls_Oracle.F_Obtiene_Datos_DM_84_AC(p_consulta)
                        lodtbDatos = cls_Oracle.FT_DATA_ACUMULACION(p_consulta, tipo_ac)
                    ElseIf cbo_tipo.SelectedValue = "OP_30" Then
                        Dim tipo_ac As String = "41"
                        lodtbDatos = cls_Oracle.FT_DATA_ACUMULACION(p_consulta, tipo_ac)
                    Else
                        lodtbDatos = cls_Oracle.F_Obtiene_Datos_DM_84(p_consulta)
                    End If
                ...
                ElseIf v_sistema = "WGS-84" Then 'PROCEDIMIENTO PARA OBTENER DM EN DATUN 84
                    If cbo_tipo.SelectedValue = "OP_29" Then
                        Dim tipo_ac As String = "1"
                        'lodbListaDM = cls_Oracle.F_SEL_LISTA_DM_HISTORICO(txtConsulta.Text, "", fecha_h & "1231")
                        'lodtbDatos = cls_Oracle.F_Obtiene_Datos_DM_84_AC(p_consulta)
                        lodtbDatos = cls_Oracle.FT_DATA_ACUMULACION(p_consulta, tipo_ac)
                    ElseIf cbo_tipo.SelectedValue = "OP_30" Then
                        Dim tipo_ac As String = "4"
                        lodtbDatos = cls_Oracle.FT_DATA_ACUMULACION(p_consulta, tipo_ac)
                    Else
                        lodtbDatos = cls_Oracle.F_Obtiene_Datos_DM_84(p_consulta)
                    End If
        ...
        Select Case lodtbDatos.Rows.Count
            ...
            Case Else
                ...
                If cbo_tipo.SelectedValue = "OP_29" Or cbo_tipo.SelectedValue = "OP_30" Then        'SE ESTA AGREGANDO "OP_30"
                ...
                ElseIf cbo_tipo.SelectedValue = "OP_29" Then
                    ...
                    v_codigo = lodtbDatos.Rows(0).Item("CG_CODIGO").ToString
                    v_zona_dm = (lodtbDatos.Rows(0).Item("PE_ZONCAT").ToString)
                    'v_zona_dm = (lodtbDatos.Rows(0).Item("PE_ZONGEO").ToString)
                    v_carta_dm = lodtbDatos.Rows(0).Item("CA_CODCAR").ToString
                    v_vigcat = lodtbDatos.Rows(0).Item("PE_VIGCAT").ToString
                    v_tipo_exp = lodtbDatos.Rows(0).Item("TE_TIPOEX").ToString
                    v_ESTADO_dm = lodtbDatos.Rows(0).Item("ESTADO").ToString
                ElseIf cbo_tipo.SelectedValue = "OP_30" Then
                    ...
                    v_vigcat = lodtbDatos.Rows(0).Item("PE_VIGCAT").ToString
                    v_tipo_exp = lodtbDatos.Rows(0).Item("TE_TIPOEX").ToString
                    v_ESTADO_dm = lodtbDatos.Rows(0).Item("EE_ESTAEX").ToString
                Else
                    v_nombre_dm = lodtbDatos.Rows(0).Item("PE_NOMDER").ToString
                    v_equivalente = lodtbDatos.Rows(0).Item("SC_CODDAT").ToString
                    v_titular_dm = lodtbDatos.Rows(0).Item("TITULAR").ToString
                ...
                'cls_Catastro.Delete_Rows_FC_GDB_Cata("Catastro_18")

                If cbo_tipo.SelectedValue = "OP_29" Then
                    fecha_archi_sim = DateTime.Now.Ticks.ToString()
                    Dim p_shapefile As String = "Acumulacion" & fecha_archi_sim & ""
                    'Try
                    '    Kill(glo_pathTMP & "Acumulacion" & "*.*")
                    'Catch ex As Exception
                    'End Try
                    cls_eval.Geoprocesamiento_temas_convert("FCToShp", p_App, p_shapefile)   ' se esta agregando para acumulacion

                    '''''cls_eval.AddFeatureClass_shp("Add", p_App, "Acumulacion")
                    'cls_Catastro.Add_ShapeFile("Acumulacion", p_App)
                    'p_shapefile = "Acumulacion" & fecha_archi_sim & ""
                    '''''cls_eval.consultacapaDM("", "Acumula", "Acumulacion")
                    'cls_eval.consultacapaDM("", "Acumula", p_shapefile)
                    '''''cls_Catastro.Exportando_Temas("Acumula", "Acumulacion", p_App)
                    'cls_Catastro.ClearLayerSelection(p_shapefile)
                    cls_Catastro.ClearLayerSelection(pFeatureLayer)
                    cls_Catastro.Quitar_Layer(p_shapefile, p_App)
                    'End If

                ElseIf cbo_tipo.SelectedValue = "OP_30" Then
                    fecha_archi_sim = DateTime.Now.Ticks.ToString()
                    Dim p_shapefile As String = "Division" & fecha_archi_sim & ""

                    cls_eval.Geoprocesamiento_temas_convert("FCToShp", p_App, p_shapefile)
                    cls_Catastro.ClearLayerSelection(pFeatureLayer)
                    cls_Catastro.Quitar_Layer(p_shapefile, p_App)

                    ' ''Dim p_shapefile As String
                    ' ''fecha_archi_sim = DateTime.Now.Ticks.ToString()
                    ' ''cls_eval.Geoprocesamiento_temas_convert("FCToShp", p_App, "Division")   ' se esta agregando para division

                    ' ''cls_eval.AddFeatureClass_shp("Add", p_App, "Division")
                    '' ''cls_Catastro.Add_ShapeFile("Acumulacion", p_App)
                    ' ''p_shapefile = "Division" & fecha_archi_sim & ""
                    ' ''cls_eval.consultacapaDM("", "Divisi", "Division")
                    '' ''cls_eval.consultacapaDM("", "Acumula", p_shapefile)
                    ' ''cls_Catastro.Exportando_Temas("Divisi", "Division", p_App)
                    '' ''cls_Catastro.ClearLayerSelection(p_shapefile)
                    ' ''cls_Catastro.ClearLayerSelection(pFeatureLayer)
                    ' ''cls_Catastro.Quitar_Layer(p_shapefile, p_App)
                End If
                ...
                If v_opcion_modulo = "OP_29" Or v_opcion_modulo = "OP_30" Then
                    gloEsteMin = xMin.Text
                    gloEsteMax = xMax.Text
                    gloNorteMin = yMin.Text
                    gloNorteMax = yMax.Text
                End If
    ...
    Private Sub cbo_tipo_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cbo_tipo.SelectedIndexChanged
        ...
        Select Case cbo_tipo.SelectedValue
            ...
            Case "OP_29", "OP_30"   'DM Acumulacion, DM Division
                ...
                cboConsulta.Items.Clear()
                cboConsulta.Enabled = True
                txtConsulta.Enabled = True     'false
                btnBuscar.Enabled = True
                lstCoordenada.Visible = True
                Txtdato5.Visible = False : Txtdato5.Enabled = False
                Txtdato6.Visible = False : Txtdato6.Enabled = False
            ...
            Case "OP_31"  'Mantenimiento DM_Acceditario

                lblSecPub.Enabled = True
                lblSecPub.Visible = True
                cboSecPub.Enabled = True
                cboSecPub.Visible = True
    ...
    Public Sub Activar_Capas_DM(ByVal p_Filtro_Capital As String, ByVal p_clbLayer As Object, ByVal p_Zona As String)
        Try
            ...
            If glo_xMin <> 0 And glo_yMin <> 0 And glo_xMax <> 0 And glo_yMax <> 0 Then
                ...
                'solo para verificar total



                ''chequear en simulado edicion no terminada
                'If v_filtro = 0 Then  'va
                '    Dim nm_depa As String = ""
                '    Dim nm_depa1 As String = ""
                ...
                '            ' lista_depa = nm_depa
                '            ' lista_depa_mod = nm_depa
                '            lista_nm_depa_veri = "NM_DEPA =  '" & nm_depa & "'"
                ...
                '            If nm_depa <> nm_depa1 Then
                '                '  lista_depa = lista_depa & "," & nm_depa
                '                lista_nm_depa_veri = lista_nm_depa_veri & " OR " & "NM_DEPA =  '" & nm_depa & "'"
                '                nm_depa1 = nm_depa
                '            End If
                '        End If
                '    Next contador
                ...
                Else
                    ...
                    If tipo_seleccion = "OP_11" Or tipo_seleccion = "OP_2" Or tipo_seleccion = "OP_16" Or tipo_seleccion = "OP_18" Or tipo_seleccion = "OP_20" Or tipo_seleccion = "OP_12" Then
                ...
                Select Case p_clbLayer.CheckedItems.Item(i) '.SelectedItem '.GetItemText(i) clbLayer.CheckedItems.Item(1)
                        ...
                        Else
                            ...
                            Else
                                If V_caso_simu = "SI" Then
                                    ...
                                    lo_Filtro_Area_Reserva = cls_Catastro.f_Intercepta_FC("Zona Reservada", v_dato1, v_dato2, v_dato3, v_dato4, m_Application)
                    ...
                    Case "Catastro Forestal"
                        ...
                        If tipo_seleccion = "OP_11" Then
                            ...
                            ' If tipo_opcion <> "2" Then 'antes estaba Para Cualquier tipo de consulta
                            cls_Catastro.PT_CargarFeatureClass_SDE(gstrFC_forestal & p_Zona, m_Application, "1", False)
                            'cls_Catastro.PT_CargarFeatureClass_SDE("GPO_ZUR_ZONA_URBANA_WGS_" & v_zona_dm, m_Application, "1", False)
                            Dim lo_Filtro_forestal As String = cls_Catastro.f_Intercepta_FC("Catastro Forestal", glo_xMin, glo_yMin, glo_xMax, glo_yMax, m_Application)
                            If lo_Filtro_forestal = "" Then
                                cls_Catastro.Quitar_Layer("Catastro Forestal", m_Application)
                            Else
                                cls_Catastro.DefinitionExpression(lo_Filtro_forestal, m_Application, "Catastro Forestal")
                                cls_Catastro.Color_Poligono_Simple(m_Application, "Catastro Forestal")
                                cls_Catastro.rotulatexto_dm("Catastro Forestal", m_Application)
                            End If
    ...
    Private Sub btnGenera_Poligono_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGenera_Poligono.Click
        ...
        If v_sistema = "PSAD-56" Then
            ...
            gstrFC_Rios = "GLI_RIO_RIOS"
            'gstrFC_Carretera = "GLI_VIA_VIAS"
            gstrFC_Carretera = "GLI_MTC_RED_VIAL_DEPARTAMENTA"
            gstrFC_CPoblado = "GPT_CPO_CENTRO_POBLADO"
            'gstrFC_Catastro_Minero = "GPO_CMI_CATASTRO_MINERO_"
        ...
        Select Case GloInt_Opcion
            Case 0
                ...
                Else
                    ...
                    If ((conta_botones_evaluacion = 1) And ((tipo_seleccion = "OP_11") Or (tipo_seleccion = "OP_12") Or (tipo_seleccion = "OP_5") Or (tipo_seleccion = "OP_6") Or (tipo_seleccion = "OP_18") Or (tipo_seleccion = "OP_20") Or (tipo_seleccion = "OP_29") Or (tipo_seleccion = "OP_30"))) Then
                        Create_Barra_2(m_Application, lodtbBotones, "Evaluación")
            ...
            Case 1
                ...
                If ((conta_botones_evaluacion = 1) And ((tipo_seleccion = "OP_11") Or (tipo_seleccion = "OP_12") Or (tipo_seleccion = "OP_5") Or (tipo_seleccion = "OP_6") Or (tipo_seleccion = "OP_18") Or (tipo_seleccion = "OP_20") Or (tipo_seleccion = "OP_29") Or (tipo_seleccion = "OP_30"))) Then
                    ...
                    Create_Barra_3(m_Application, lodtbBotones, "Acumulación_División_Renuncia")
    ...
    Public Sub Create_Barra_3(ByVal m_application As IApplication, ByVal p_Grilla As DataTable, ByVal p_NomVentana As String)
        Dim pMDocumento As IDocument
        Dim pTool_Bars As ICommandBars
        Dim pTool_Boton As ICommandBar = Nothing
        pMDocumento = m_application.Document
    ...
    Private Sub btn_procesar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_procesar.Click
        ...
        'captura fecha
        pubfec_acceditario = Microsoft.VisualBasic.Left(Format(dtpFecha1.Value), 10)
        cls_Catastro.Borra_Todo_Feature("", m_Application)
        ...
        Select Case Me.cbo_tipo.SelectedValue
            ...
            Case "OP_31" 'Mantenimiento DM_Acceditario
                ...
                Dim v_dmacceditario As New DataTable
                Dim tipo As String = "1"

                'v_dmacceditario = cls_Oracle.FT_DATA_ACCEDITARIO(txtSecPub.Text, tipo)
                v_dmacceditario = cls_Oracle.FT_DATA_ACCEDITARIO(v_secpub, tipo)
                Dim codi_acced As String
                If v_dmacceditario.Rows.Count > 0 Then
                    For contador As Integer = 0 To v_dmacceditario.Rows.Count - 1
                        codi_acced = v_dmacceditario.Rows(contador).Item("CG_CODIGO")
                        If contador = 0 Then
                            lista_codigo_acce = "CODIGOU =  '" & codi_acced & "'"
                        ElseIf contador > 0 Then
                            lista_codigo_acce = lista_codigo_acce & " OR " & "CODIGOU =  '" & codi_acced & "'"
                        End If
                    Next contador

                    For datum As Integer = 1 To 2
                        For i As Integer = 17 To 19
                            If i = 17 Then
                                v_zona_dm = "17"
                            ElseIf i = 18 Then
                            ...
                            If datum = 1 Then
                                ...
                                cls_eval.consultacapaDM("", "Acceditario", "Catastro")
                                cls_Catastro.Expor_Tema("Accedi_p" & i, sele_denu, m_Application)
                                'cls_Catastro.ClearLayerSelection(pFeatureLayer)
                                cls_Catastro.Add_ShapeFile("Accedi_p" & i, m_Application)
                                cls_eval.agregacampotema_tpm("Acceditario", "Acceditario") 'Agrega campo 
                                cls_Catastro.Update_Value_acceditario(m_Application, "Acceditario")  'registra fecha
                                cls_Catastro.Quitar_Layer("Acceditario", m_Application)  'quita la capa
                            Else
                                gstrFC_Catastro_Minero = "GPO_CMI_CATASTRO_MINERO_WGS_"
                                cls_Catastro.PT_CargarFeatureClass_SDE(gstrFC_Catastro_Minero & v_zona_dm, m_Application, "1", False)
                                'cls_eval.consultacapaDM("", "Acceditario", "Catastro")
                                cls_eval.consultacapaDM("Acceditario", "Acceditario", "Catastro")
                                cls_Catastro.Expor_Tema("Accedi_w" & i, sele_denu, m_Application)
                                'cls_Catastro.ClearLayerSelection(pFeatureLayer)
                                cls_Catastro.Add_ShapeFile("Accedi_w" & i, m_Application)
                                cls_eval.agregacampotema_tpm("Acceditario", "Acceditario") 'Agrega campo 
                                cls_Catastro.Update_Value_acceditario(m_Application, "Acceditario")  'registra fecha
                                cls_Catastro.Quitar_Layer("Acceditario", m_Application)
                            End If
                        Next i
                    Next datum

                    Dim RetVal
                    While RetVal Is Nothing   ' Agrega registros a las capas de acceditarios
                        '  glo_Path_EXE = "C:\BDGEOCATMIN\Temporal\Acceditario\"
                        glo_Path_EXE = "U:\DATOS\DBF\"  ' para que el usuario no borre los programas por casualidad

                        RetVal = Shell(glo_Path_EXE & "copyacceditarios.bat", 1, True)  'copia los individuales a otra carpeta
                    End While

                    MsgBox("Parte 1:  Generando acceditarios individuales para integrarlos a la base total...", MsgBoxStyle.Exclamation, "SIGCATMIN")

                    Dim RetVal1
                    While RetVal1 Is Nothing   ' Agrega registros a las capas de acceditarios
                        ' glo_Path_EXE = "C:\BDGEOCATMIN\Temporal\Acceditario\"
                        glo_Path_EXE = "U:\DATOS\DBF\\"
                        RetVal1 = Shell(glo_Path_EXE & "acceditario.bat", 1, True)  'espera mientras termina para ejecutar siguiente linea
                    End While
                    ...
                    'While RetVal Is Nothing   ' Agrega registros a las capas de acceditarios
                    '    glo_Path_EXE = "C:\BDGEOCATMIN\Temporal\Acceditario\"
                    '    RetVal = Shell(glo_Path_EXE & "acceditario.bat", 1, True)  'espera mientras termina para ejecutar siguiente linea
                    'End While
                    'Dim v_numdmacce As String
                    '''''v_numdmacce = cls_Oracle.FT_ELIMINA_DM_ACCEDITARIO(txtSecPub.Text)
                    'v_numdmacce = cls_Oracle.FT_ELIMINA_DM_ACCEDITARIO(v_secpub)

                    'Dim RetValcopy
                    'While RetValcopy Is Nothing   ' copia capas de acceditarios de la GDB a U:
                    '    glo_Path_EXE = "C:\BDGEOCATMIN\Temporal\Acceditario\"
                    '    RetValcopy = Shell(glo_Path_EXE & "accedicopy.bat", 1, True)  'espera mientras termina para ejecutar siguiente linea
                    'End While

                    cls_Catastro.Borra_Todo_Feature("", m_Application)
                    MsgBox("Proceso Terminado de Actualización de Acceditarios...", MsgBoxStyle.Exclamation, "SIGCATMIN")
                Else
                    MsgBox("No existen Acceditarios para Actualizar en la base...", MsgBoxStyle.Critical, "SIGCATMIN")
                    Exit Sub
                End If
    ...
    Private Sub img_DM1_Click(sender As Object, e As EventArgs) Handles img_DM1.Click

    End Sub

    Private Sub dgdResultado_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgdResultado.CellContentClick

    End Sub

    Private Sub cboSecPub_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cboSecPub.SelectedIndexChanged

        If Me.cboSecPub.SelectedValue.ToString = "" Then
            MsgBox("Debe seleccionar la Secuencia de Publicación")
        Else
            v_secpub = Me.cboSecPub.SelectedValue.ToString
        End If

        'v_secpub = Me.cboSecPub.SelectedItem.ToString
        'v_secpub = Me.cboSecPub.SelectedValue.ToString
